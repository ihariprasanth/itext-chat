...
if (btnAttach && attachMenu) {
  ...
  // Attach item clicks (hook up your file-picker logic here)
  attachMenu.querySelectorAll(".attach-item").forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      if (type === "document") input.accept = ".pdf,.doc,.docx,.txt,.xls,.xlsx,.ppt,.pptx";
      input.click();
    });
  });
}
...
function updateMemberBar(count, max = 100) {
  const fill  = $("member-bar-fill");
  const label = $("member-bar-label");
  if (fill)  fill.style.width  = `${Math.min(100, (count / max) * 100)}%`;
  if (label) label.textContent = `${count} / ${max}`;
}

// Patch renderUserList to also update member bar
const _origRenderUserList = renderUserList;
// Note: renderUserList is already defined above and calls updateMemberBar via patching
// We re-export a wrapper since renderUserList is in the same scope:
// (The existing renderUserList will call updateMemberBar below)
